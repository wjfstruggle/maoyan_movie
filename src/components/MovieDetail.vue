...
<script>
import qs from 'qs';
import myCinema from '@/components/myCinema.vue'
export default {
    created(){
    ...
    methods: {
       ...
       format() {
                   ...
                   for (let i = 0; i < 7; i++) {
					...
					for(var i=1;i<7;i++){
						...
						if(i==1){
							...
							this.showDays.push("后天 "+resentYear+'-'+resentMonth+'-'+resentDay)
						}else{
           ...
           }else if((new Date(this.showDays[0])).getTime()<(new Date().getTime())){
               //电影上映的那天比今天早
					...
					if(days-nowDay>=7){
						...
						for(var i=1;i<7;i++){
							...
							if(i==1){
								this.showDays.push("今天 "+nowYear+"-"+nowMonth+"-"+(nowDay-1))
								this.showDays.push("明天 "+nowYear+"-"+nowMonth+"-"+nowDay)
							}else if(i==2){
