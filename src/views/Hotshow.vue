                ...
                <div class="movie_wrapper">
                    <div class="movie_detail">
                        <div class="title_name">
                            <h1 class="title">{{movie.nm}}</h1>
                            <span class="icon" v-if="movie.version =='v3d imax'" :class="{'active1': movie.version =='v3d imax'}"></span>
                            <span v-else class="icon" :class="{'active2': movie.version =='v2d imax'}"></span>
                        </div>
                        <p class="evaluate" v-if="movie.sc > 0 && movie.preShow">观众评:<span>{{movie.sc}}</span></p>
                        <p class="evaluate" v-else-if="movie.sc == 0">暂无评分</p>
                        <p class="evaluate" v-else><span>{{movie.wish}}</span>人想看</p>
                        <h5 class="people textOverflow1">主演:{{movie.star}}</h5>
                        <h6 class="show">{{movie.showInfo}}</h6>
                    </div>
                </div>
                <!-- 购票预售组件 -->
                <div class="btn" >
...
export default {
  ...
  created() {
    ...
    this.$store.commit('changeHt','狗眼电影');
  },
  data() {
      return {
        movieList: [],
